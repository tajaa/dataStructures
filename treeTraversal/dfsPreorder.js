...
// DFS Pre Order Pseudo code
// 1. Create a variable to store values we’ve visited and return that call it ‘data’ or ‘visited’
// 2. Store the root of the BST in a variable called ‘current’
// 3. Write a helper function which accepts a node
//     1. Push the value of the node to the variable that stores the values
//     2. If the node has a let property, call the helper function with the left property on the node
//     3. If the node has a right property, call the helper function with the right property on the node
// 4. Invoke the helper function with the current variable
// 5. Return the array of values

// DFS-POST ORDER : RECURSIVELY
// 1. Create a variable to store the values of nodes visited
// 2. Store the root of the of the BST in a variable called current
// 3. Write a helper function which accepts a node
//     1. If the node has a left property, call the helper function with the left property on the node
//     2. If the nose has a right property, call the helper function with the right property on that node
...
//once the program traversers and pushes for a side
//it holds that in memory so when it come sback, it doesn't
//infinite loop. Works its way recursively back to 10 and then
//checks the right. The nodes have values in left and right so
//they don't do those operations again.

// expected output [3,8,6, 20, 15, 10]
